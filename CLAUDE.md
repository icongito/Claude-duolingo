...
**Economy & monetization** (`src/lib/data/economy.ts`): single source for plans (Free/Premium/Premium+), gem packs, consumables, wearables, and quests. Fair-play invariants are unit-tested (`economy.test.ts`): daily quests never pay gems, weekly recurring gem income ≤ `WEEKLY_GEM_CAP`, and nothing purchasable grants answers/content/rank. Mentor caps are **cost-derived**: `MENTOR_MODELS` holds Claude API list pricing, and tests assert each tier's worst-case monthly mentor spend stays ≤ `MENTOR_COST_CEILING_PCT` of its yearly-rate revenue (free tier ≤ assumed ads ARPU) — change a price or cap and the tests referee it. The free tier is ad-supported: `plan.showsAds` gates `components/ads/ad-card.tsx` — one card at the end of every lesson session (passed, failed, and boss victory/defeat screens); paid plans render nothing. Screens: `/premium` (tier cards + gem packs + FAQ), `/premium/checkout` (zod-validated demo card form — no processor connected; production swaps in Stripe), `/settings/billing` (plan, card, invoices, cancel dialog), `/quests` (daily coins; weekly + special quests are the gem faucets). The mentor route picks its Claude model and daily message limit from the plan config.

**Boss battles** (`src/lib/data/boss.ts` + `components/lesson/boss-player.tsx`): the last node of every world opens a named 3-stage gauntlet (Recall/Rebuild/Prove It) built from the world's lesson steps — shared countdown (45s per attack), 3 hearts across all stages, boss HP drops only on correct answers (misses repeat the step), 250 XP + 10-gem first clear. `getBossBattle(nodeId)` returns null for non-boss nodes; the lesson page falls through to the regular player.
...
**Mascot.** The canonical mascot ("Little Guy") is a wide terracotta pixel creature with two square eyes, side arm nubs, and four legs. The in-app component is `src/components/mascot/little-guy.tsx` — a transparent-background 64x48 canvas engine with a parametric `guyCells()` rig and nine named animations (`<LittleGuy animation="lesson" size={220} />`); it freezes on a still frame under `prefers-reduced-motion`. Wearables (`wear="cap" | "wizard" | "crown" | "specs"`) are extra cell layers drawn inside `guyCells()` so they track every animation; the purchasable catalog in `economy.ts` must stay in sync with the `LittleGuyWear` union. `public/little-guy.html` is the standalone light/dark showcase of the same engine. Any mascot rendering must keep the four-leg silhouette and hard pixel edges (`image-rendering: pixelated`).
